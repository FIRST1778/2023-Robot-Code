...
import org.frc1778.lib.pathplanner.simple.JSONArray;
...
public class PathPlanner {
  ...
  public static PathPlannerTrajectory loadPath(
    ...
    try (BufferedReader br =
      ...
      List<PathPlannerTrajectory.Waypoint> waypoints = getWaypointsFromJson(json);
  ...
  public static List<PathPlannerTrajectory> loadPathGroup(
    ...
    try (BufferedReader br =
      ...
      List<List<PathPlannerTrajectory.Waypoint>> splitWaypoints = new ArrayList<>();
      List<List<PathPlannerTrajectory.EventMarker>> splitMarkers = new ArrayList<>();

      List<PathPlannerTrajectory.Waypoint> currentPath = new ArrayList<>();
      for (int i = 0; i < waypoints.size(); i++) {
        ...
        if (w.isStopPoint || i == waypoints.size() - 1) {
          ...
          List<PathPlannerTrajectory.EventMarker> currentMarkers = new ArrayList<>();
          for (PathPlannerTrajectory.EventMarker marker : markers) {
            if (marker.waypointRelativePos >= waypoints.indexOf(currentPath.get(0))
              ...
              currentMarkers.add(
                  new PathPlannerTrajectory.EventMarker(
                      marker.names,
  ...
  public static PathPlannerTrajectory generatePath(
      ...
      List<PathPlannerTrajectory.EventMarker> eventMarkers) {
    if (points.size() < 2) {
    ...
    waypoints.add(
        new PathPlannerTrajectory.Waypoint(
            firstPoint.position,
            null,
            null,
  ...
  public static PathPlannerTrajectory generatePath(
          PathConstraints constraints, List<PathPoint> points, List<PathPlannerTrajectory.EventMarker> markers) {
  ...
  private static List<PathPlannerTrajectory.EventMarker> getMarkersFromJson(JSONObject json) {
    JSONArray jsonMarkers = (JSONArray) json.get("markers");

    List<PathPlannerTrajectory.EventMarker> markers = new ArrayList<>();
    ...
    if (jsonMarkers != null) {
      for (Object marker : jsonMarkers) {
        ...
        markers.add(new PathPlannerTrajectory.EventMarker(names, ((Number) jsonMarker.get("position")).doubleValue()));
      }
    }
