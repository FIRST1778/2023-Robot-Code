...
public class PathPlannerTrajectory extends Trajectory {
    ...
    public PathPlannerTrajectory(List<Waypoint> pathPoints, List<EventMarker> markers, PathConstraints constraints, boolean reversed, boolean fromGUI) {
    ...
    public PathPlannerTrajectory(List<State> states, List<EventMarker> markers, StopEvent startStopEvent, StopEvent endStopEvent, boolean fromGUI) {
        super(states);
    ...
    public static PathPlannerState transformStateForAlliance(PathPlannerState state, DriverStation.Alliance alliance) {
        if (alliance == DriverStation.Alliance.Red) {
            // Create a new state so that we don't overwrite the original
            PathPlannerState transformedState = new PathPlannerState();

            Translation2d transformedTranslation = new Translation2d(state.poseMeters.getX(), FIELD_WIDTH_METERS - state.poseMeters.getY());
    ...
    public static PathPlannerTrajectory transformTrajectoryForAlliance(PathPlannerTrajectory trajectory, DriverStation.Alliance from, DriverStation.Alliance to) {
        if (to != from) {
            List<State> transformedStates = new ArrayList<>();
            ...
            for (State s : trajectory.getStates()) {
                ...
                transformedStates.add(transformStateForAlliance(state, from, to));
            }
    ...
    public static PathPlannerState transformStateForAlliance(PathPlannerState state, DriverStation.Alliance from, DriverStation.Alliance to) {
        if (to != from) {
            // Create a new state so that we don't overwrite the original
            int transformDirection;
            if (to == DriverStation.Alliance.Red) {
            ...
            Translation2d transformedTranslation = new Translation2d((FIELD_LENGTH_METERS / 2) + (transformDirection * Math.abs((FIELD_LENGTH_METERS / 2) - state.poseMeters.getX())), state.poseMeters.getY());
            Rotation2d transformedHeading = state.poseMeters.getRotation().times(-1);
    ...
    public static List<Waypoint> transformWaypointsForAlliance(List<Waypoint> waypoints, DriverStation.Alliance to, DriverStation.Alliance from) {
        if (from != to) {
            int transformDirection;
            if (to == DriverStation.Alliance.Red) {
                transformDirection = 1;
            ...
            List<Waypoint> transformedWaypoints = new ArrayList<Waypoint>();
            for (Waypoint waypoint : waypoints) {
                Translation2d transformedAnchorPoint = new Translation2d((FIELD_LENGTH_METERS / 2) + (transformDirection * ((FIELD_LENGTH_METERS / 2) - waypoint.anchorPoint.getX())), waypoint.anchorPoint.getY());
                Rotation2d transformedHolonomicRotation = waypoint.holonomicRotation.plus(Rotation2d.fromDegrees(180.0)).times(-1);

                Translation2d transformedNextControl = null;
                if (waypoint.nextControl != null) {
                    transformedNextControl = new Translation2d((FIELD_LENGTH_METERS / 2) + (transformDirection * ((FIELD_LENGTH_METERS / 2) - waypoint.nextControl.getX())), waypoint.nextControl.getY());
                }

                Translation2d transformedPrevControl = null;
                if (waypoint.prevControl != null) {
                    transformedPrevControl = new Translation2d((FIELD_LENGTH_METERS / 2) + (transformDirection * ((FIELD_LENGTH_METERS / 2) - waypoint.prevControl.getX())), waypoint.prevControl.getY());
                }
                transformedWaypoints.add(new Waypoint(transformedAnchorPoint, transformedNextControl, transformedNextControl, waypoint.velOverride, transformedHolonomicRotation, waypoint.isReversal, waypoint.isStopPoint, waypoint.stopEvent));
    ...
    private static List<State> generatePath(List<Waypoint> pathPoints, double maxVel, double maxAccel, boolean reversed) {
    ...
    private static void calculateVelocity(List<PathPlannerState> states, List<Waypoint> pathPoints, double maxAccel) {
    ...
    private static List<PathPlannerState> joinSplines(List<Waypoint> pathPoints, double maxVel, double step) {
    ...
    public static class Waypoint {
        ...
        public Waypoint(Translation2d anchorPoint, Translation2d prevControl, Translation2d nextControl, double velOverride, Rotation2d holonomicRotation, boolean isReversal, boolean isStopPoint, StopEvent stopEvent) {
    ...
    public static class StopEvent {
        ...
        public enum WaitBehavior {
            NONE("none"), BEFORE("before"), AFTER("after"), DEADLINE("deadline"), MINIMUM("minimum");
        ...
        public StopEvent(List<String> names, ExecutionBehavior executionBehavior, WaitBehavior waitBehavior, double waitTime) {
            this.names = names;
